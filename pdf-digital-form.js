...
function parseKids(pdfParser, fieldDictionary, inheritedProperties, baseFieldName) {
  ...
  if (fieldDictionary.exists('DA')) localEnv['DA'] = toText(fieldDictionary.queryObject('DA'));
  if (fieldDictionary.exists('Opt')) localEnv['Opt'] = fieldDictionary.queryObject('Opt').toPDFArray();

  // parse kids
  var result = parseFieldsArray(
    pdfParser,
...
function parseTextFieldValue(pdfParser, fieldDictionary, fieldName) {
  ...
  if (!fieldDictionary.exists(fieldName)) return null;
...
function parseFieldsValueData(result, pdfParser, fieldDictionary, flags, inheritedProperties) {
  ...
  if (!fieldType) return null; // k. must be a widget
...
function parseField(pdfParser, fieldDictionary, inheritedProperties, baseFieldName) {
  ...
  if (flags === undefined || flags === null) flags = 0;
...
function parseFieldsArray(pdfParser, fieldsArray, inheritedProperties, baseFieldName) {
  ...
  for (var i = 0; i < fieldsArray.getLength(); ++i) {
    var fieldResult = parseField(
      pdfParser,
      pdfParser.queryArrayObject(fieldsArray, i).toPDFDictionary(),
      inheritedProperties,
      baseFieldName
    );
    if (fieldResult) result.push(fieldResult);
  }
...
function accumulateFieldsValues(result, fieldsArray) {
  fieldsArray.forEach(function (field) {
    if (field.kids) accumulateFieldsValues(result, field.kids);
    else result[field.fullName] = field.value;
  });
}
...
function PDFDigitalForm(pdfParser) {
  ...
  if (this.acroformDict) {
    var fieldsArray = this.acroformDict.exists('Fields') ? pdfParser.queryDictionaryObject(this.acroformDict, 'Fields').toPDFArray() : null;
    if (fieldsArray) this.fields = parseFieldsArray(pdfParser, fieldsArray, {}, '');
  }
}
